...
\title{估计量的一致性与渐近正态性 \\ \large Consistency and asymptotic normality of estimators}
...
本章对应原文 \textit{Consistency and asymptotic normality of estimators}。首版讲义先按照原文的章节主线整理核心概念、教学解释和 Python 工程实践；后续精修时，可继续把原 PDF 中的证明、练习和细节推导逐节扩展进来。

本章范围：收敛模式、抽样性质、强相合、依概率收敛、渐近正态和 GMLE 性质。

\section{核心知识点}

\begin{itemize}
  \item 一致性说明样本量增大时估计量靠近真值。
  \item 渐近正态性为置信区间和检验提供近似依据。
  \item 时间序列中的依赖条件决定大数定律和中心极限定理能否使用。
\end{itemize}
...
\begin{figure}[htbp]
  ...
  \caption{估计量的一致性与渐近正态性 Python 实践图：ch14\_fig01\_consistency\_mse.png}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.86\textwidth]{ch14_fig02_asymptotic_normality.png}
  \caption{估计量的一致性与渐近正态性 Python 实践图：ch14\_fig02\_asymptotic\_normality.png}
\end{figure}
