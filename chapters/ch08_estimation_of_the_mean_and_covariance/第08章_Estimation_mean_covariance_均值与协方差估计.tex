...
\title{均值与协方差估计 \\ \large Estimation of the mean and covariance}
...
本章对应原文 \textit{Estimation of the mean and covariance}。首版讲义先按照原文的章节主线整理核心概念、教学解释和 Python 工程实践；后续精修时，可继续把原 PDF 中的证明、练习和细节推导逐节扩展进来。

本章范围：样本均值、样本自协方差、相关性检验、Newey-West 和拟合优度。

\section{核心知识点}

\begin{itemize}
  \item 样本均值在相关序列下仍常用，但标准误不同于 iid 情形。
  \item 样本 ACF 是检测相关结构的基本工具。
  \item HAC/Newey-West 估计用于处理自相关和异方差下的标准误。
\end{itemize}
...
\begin{figure}[htbp]
  ...
  \caption{均值与协方差估计 Python 实践图：ch08\_fig01\_sample\_mean\_dependence.png}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.86\textwidth]{ch08_fig02_sample_acf.png}
  \caption{均值与协方差估计 Python 实践图：ch08\_fig02\_sample\_acf.png}
\end{figure}
