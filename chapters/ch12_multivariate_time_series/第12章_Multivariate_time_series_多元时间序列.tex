...
\title{多元时间序列 \\ \large Multivariate time series}
...
本章对应原文 \textit{Multivariate time series}。首版讲义先按照原文的章节主线整理核心概念、教学解释和 Python 工程实践；后续精修时，可继续把原 PDF 中的证明、练习和细节推导逐节扩展进来。

本章范围：多元序列回归、条件独立、相干性、谱密度矩阵和逆谱密度矩阵。

\section{核心知识点}

\begin{itemize}
  \item 多元时间序列关注多个序列之间的动态相互作用。
  \item 相干性是频域中的相关性度量。
  \item 逆谱密度矩阵用于描述条件依赖结构。
\end{itemize}
...
\begin{figure}[htbp]
  ...
  \caption{多元时间序列 Python 实践图：ch12\_fig01\_var\_series.png}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.86\textwidth]{ch12_fig02_coherence.png}
  \caption{多元时间序列 Python 实践图：ch12\_fig02\_coherence.png}
\end{figure}
