...
\title{参数估计 \\ \large Parameter estimation}
...
本章对应原文 \textit{Parameter estimation}。首版讲义先按照原文的章节主线整理核心概念、教学解释和 Python 工程实践；后续精修时，可继续把原 PDF 中的证明、练习和细节推导逐节扩展进来。

本章范围：AR 与 ARMA 参数估计、Yule-Walker、高斯似然、条件似然、Burg 和 Hannan-Rissanen。

\section{核心知识点}

\begin{itemize}
  \item Yule-Walker 估计把自协方差方程转化为参数估计。
  \item 条件最小二乘适合把 AR 模型写成回归问题。
  \item 最大似然方法给出统一框架，但计算复杂度更高。
\end{itemize}
...
\begin{figure}[htbp]
  ...
  \caption{参数估计 Python 实践图：ch09\_fig01\_yule\_walker\_sampling.png}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.86\textwidth]{ch09_fig02_cls_ar1.png}
  \caption{参数估计 Python 实践图：ch09\_fig02\_cls\_ar1.png}
\end{figure}
