...
本章对应原文 \textit{Background}。首版讲义先按照原文的章节主线整理核心概念、教学解释和 Python 工程实践；后续精修时，可继续把原 PDF 中的证明、练习和细节推导逐节扩展进来。

本章范围：不等式、鞅、傅里叶级数、Burkholder 不等式和 FFT。

\section{核心知识点}

\begin{itemize}
  \item 附录 A 提供正文推导所需的概率和分析工具。
  \item 傅里叶级数解释周期函数的频率分解。
  \item FFT 是频域计算的工程基础。
\end{itemize}
...
\begin{figure}[htbp]
  ...
  \caption{背景知识 Python 实践图：appA\_fig01\_fourier\_series.png}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.86\textwidth]{appA_fig02_fft_energy.png}
  \caption{背景知识 Python 实践图：appA\_fig02\_fft\_energy.png}
\end{figure}
