...
本章对应原文 \textit{Prediction}。首版讲义先按照原文的章节主线整理核心概念、教学解释和 Python 工程实践；后续精修时，可继续把原 PDF 中的证明、练习和细节推导逐节扩展进来。

本章范围：自回归预测、有限过去预测、Durbin-Levinson、Kalman 滤波和非线性预测。

\section{核心知识点}

\begin{itemize}
  \item 预测利用过去信息构造未来观测的条件均值或最佳线性预测。
  \item AR 模型预测可以递归计算。
  \item Kalman 滤波把 ARMA 预测放入状态空间框架。
\end{itemize}
...
\begin{figure}[htbp]
  ...
  \caption{预测 Python 实践图：ch07\_fig01\_ar\_forecast.png}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.86\textwidth]{ch07_fig02_forecast_errors.png}
  \caption{预测 Python 实践图：ch07\_fig02\_forecast\_errors.png}
\end{figure}
