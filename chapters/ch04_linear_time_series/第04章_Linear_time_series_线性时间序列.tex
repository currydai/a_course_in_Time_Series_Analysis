...
\title{线性时间序列 \\ \large Linear time series}
...
本章对应原文 \textit{Linear time series}。首版讲义先按照原文的章节主线整理核心概念、教学解释和 Python 工程实践；后续精修时，可继续把原 PDF 中的证明、练习和细节推导逐节扩展进来。

本章范围：线性过程、MA、AR、ARMA、单位根、模拟和诊断。

\section{核心知识点}

\begin{itemize}
  \item AR 模型把当前值表示为过去值和新冲击的组合。
  \item MA 模型把当前值表示为当前和过去冲击的有限或无限组合。
  \item ACF/PACF 是识别 AR 与 MA 结构的核心诊断图。
\end{itemize}
...
\begin{figure}[htbp]
  ...
  \caption{线性时间序列 Python 实践图：ch04\_fig01\_arma\_series.png}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.86\textwidth]{ch04_fig02_acf_pacf.png}
  \caption{线性时间序列 Python 实践图：ch04\_fig02\_acf\_pacf.png}
\end{figure}
