...
\title{平稳时间序列的自协方差与偏协方差 \\ \large The autocovariance and partial covariance of a stationary time series}
...
本章对应原文 \textit{The autocovariance and partial covariance of a stationary time series}。首版讲义先按照原文的章节主线整理核心概念、教学解释和 Python 工程实践；后续精修时，可继续把原 PDF 中的证明、练习和细节推导逐节扩展进来。

本章范围：自协方差函数、Yule-Walker 方程、样本 ACF、偏自相关和模型识别。

\section{核心知识点}

\begin{itemize}
  \item 自协方差函数描述同一序列不同滞后之间的线性依赖。
  \item AR 模型的自协方差满足 Yule-Walker 方程。
  \item PACF 对 AR 阶数识别特别有用。
\end{itemize}
...
\begin{figure}[htbp]
  ...
  \caption{平稳时间序列的自协方差与偏协方差 Python 实践图：ch06\_fig01\_acf\_shapes.png}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.86\textwidth]{ch06_fig02_pacf_shapes.png}
  \caption{平稳时间序列的自协方差与偏协方差 Python 实践图：ch06\_fig02\_pacf\_shapes.png}
\end{figure}
